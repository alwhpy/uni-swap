...
function useTokenFromActiveNetwork(tokenAddress: string | undefined): Token | null | undefined {
  ...
  const tokenName = useSingleCallResult(chainId, tokenContract, 'name', undefined, NEVER_RELOAD)
  const tokenNameBytes32 = useSingleCallResult(chainId, tokenContractBytes32, 'name', undefined, NEVER_RELOAD)
  const symbol = useSingleCallResult(chainId, tokenContract, 'symbol', undefined, NEVER_RELOAD)
  const symbolBytes32 = useSingleCallResult(chainId, tokenContractBytes32, 'symbol', undefined, NEVER_RELOAD)
  const decimals = useSingleCallResult(chainId, tokenContract, 'decimals', undefined, NEVER_RELOAD)
...
function useCurrencyFromMap(
  tokens: TokenMap,
  chainId: ChainId | undefined | SupportedChainId,
  currencyId?: string | null
): Currency | undefined {
  const nativeCurrency = useNativeCurrency(chainId)
  const isNative = Boolean(
    nativeCurrency && (currencyId?.toUpperCase() === 'ETH' || currencyId?.toUpperCase() === 'BB')
  )
...
function useTokensFromMap(
  ...
  chainId: Maybe<ChainId | SupportedChainId>
...
const allTokensFromLists = {} as any
export function useAllTokensMultichain(): ChainTokenMap {
  return useMemo(() => {
    ...
    Object.keys(allTokensFromLists).forEach(key => {
      ...
      Object.values(allTokensFromLists[chainId]).forEach(({ token }: any) => {
...
export function useDefaultActiveTokens(chainId: Maybe<SupportedChainId>): { [address: string]: Token } {
  const defaultListTokens = DEFAULT_LIST_OF_LISTS as any
...
>
const listsByUrl = {} as any
const unsupportedTokensMap = {}
export function useUnsupportedTokens(): { [address: string]: Token } {
  ...
  const l2InferredBlockedTokens: typeof unsupportedTokens = useMemo(() => {
    ...
    return list.tokens.reduce((acc: any, tokenInfo: { extensions: { bridgeInfo: unknown }; decimals: number }) => {
...
function useTokenListCurrency(currencyId: Maybe<string>, chainId?: SupportedChainId): Currency | undefined {
...
export function useCurrency(address?: string, chainId?: ChainId | SupportedChainId, skip?: boolean): Maybe<Currency> {
  const { chainId: connectedChainId } = useActiveWeb3React()
