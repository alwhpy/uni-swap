...
export default function useClassicAutoSlippageTolerance(trade?: ClassicTrade): Percent {
  ...
  const onL2 = chainId && (L2_CHAIN_IDS as any).includes(chainId)
  ...
  const supportsGasEstimate = useMemo(
    () => chainId && (SUPPORTED_GAS_ESTIMATE_CHAIN_IDS as any).includes(chainId),
    [chainId]
  )
  ...
  return useMemo(() => {
    ...
    const dollarCostToUse =
      chainId && (SUPPORTED_GAS_ESTIMATE_CHAIN_IDS as any).includes(chainId) && gasEstimateUSD
        ? gasEstimateUSD
