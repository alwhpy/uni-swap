...
function getTitle({
  ...
  if (isLimitTrade(trade)) {
    if (swapPending) return `Limit submitted`
    if (swapConfirmed) return `Limit filled!`

    return `Confirm limi`
  }

  if (swapPending) return `Swap submitted`
  if (swapConfirmed) return `Swap success!`
...
export function Pending({
  ...
  const showSubmitted = swapPending && !swapConfirmed && chainId === SupportedChainId.BIT_DEVNET
  const showSuccess = swapConfirmed || (chainId !== SupportedChainId.BIT_DEVNET && swapPending)
  ...
  const explorerLink = useMemo(() => {
    ...
    return getEtherscanLink(chainId || (ChainId.MAINNET as any), txHash, ExplorerDataType.TRANSACTION)
      ...
      <HeaderContainer gap="md" $disabled={transactionPending && !limitPlaced}>
        ...
        {!swapPending && !swapConfirmed && (
          <Row justify="center" marginTop="32px" minHeight="24px">
            <ThemedText.BodySmall color="neutral2">{`Proceed in your wallet`}</ThemedText.BodySmall>
          </Row>
        )}
