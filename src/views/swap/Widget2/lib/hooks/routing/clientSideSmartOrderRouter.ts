...
import { AlphaRouter, AlphaRouterConfig, V2SubgraphProvider, V3SubgraphProvider } from '@uniswap/smart-order-router'
...
export const chainProtocols: {
  [x in ChainId]?: {
    [Protocol.V3]: V3SubgraphProvider
    [Protocol.V2]: V2SubgraphProvider
  }
} = {
  // [ChainId.BIT_MAINNET]: {
  //   [Protocol.V3]: new V3SubgraphProvider(ChainId.BIT_MAINNET, 3, 90000),
  //   [Protocol.V2]: new V2SubgraphProvider(ChainId.BIT_MAINNET, 3, 900000, true, 1000)
  // }
}
...
export function getRouter(chainId: ChainId): AlphaRouter {
  ...
  if (supportedChainId) {
    ...
    const router = new AlphaRouter({
      chainId,
      provider,
      v3SubgraphProvider: chainProtocols[chainId]?.[Protocol.V3],
      v2SubgraphProvider: chainProtocols[chainId]?.[Protocol.V2]
    })
