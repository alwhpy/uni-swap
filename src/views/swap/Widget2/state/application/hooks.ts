...
export function useModalIsOpen(modal: ApplicationModal): boolean {
  const openModal = useAppSelector((state: AppState) => state.swap2.application.openModal)
...
async function getMoonpayAvailability(): Promise<boolean> {
  const moonpayPublishableKey = process.env.NEXT_PUBLIC_REACT_APP_MOONPAY_PUBLISHABLE_KEY
  if (!moonpayPublishableKey) {
    throw new Error('Must provide a publishable key for moonpay.')
  }
  const moonpayApiURI = process.env.NEXT_PUBLIC_REACT_APP_MOONPAY_API
  if (!moonpayApiURI) {
...
export function useFiatOnrampAvailability(shouldCheck: boolean, callback?: () => void) {
  ...
  const { available, availabilityChecked } = useAppSelector((state: AppState) => state.swap2.application.fiatOnramp)
...
export function useCloseModal() {
  const dispatch = useAppDispatch()
  const currentlyOpenModal = useAppSelector((state: AppState) => state.swap2.application.openModal)
  return useCallback(
