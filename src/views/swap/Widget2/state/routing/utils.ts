...
export function transformQuickRouteToTrade(args: GetQuickQuoteArgs, data: QuickRouteResponse): PreviewTrade {
  ...
  const [rawAmountIn, rawAmountOut] = data.tradeType === 'EXACT_IN' ? [amount, data.quote] : [data.quote, amount]
  const inputAmount = CurrencyAmount.fromRawAmount(currencyIn, rawAmountIn as string)
  const outputAmount = CurrencyAmount.fromRawAmount(currencyOut, rawAmountOut as string)
