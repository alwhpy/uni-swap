...
const CLIENT_PARAMS: Partial<AlphaRouterConfig> = {
    ...
    getQuote: build.query<TradeResult, GetQuoteArgs>({
      async queryFn(args, _api, _extraOptions, fetch) {
        ...
        try {
          ...
          return { data: { ...tradeResult, latencyMs: Date.now() } }
        } catch (error: any) {
          console.warn(
        ...
        try {
          ...
          const quoteResult = await getClientSideQuote(args, router, CLIENT_PARAMS, args.gasPrice)
          if (quoteResult.state === QuoteState.SUCCESS) {
            const trade = await transformQuoteToTrade(args, quoteResult.data, QuoteMethod.CLIENT_SIDE_FALLBACK)
            return {
              data: { ...trade, latencyMs: Date.now() }
            }
          } else {
            return { data: { ...quoteResult, latencyMs: Date.now() } }
          }
        } catch (error: any) {
          console.error(error)
