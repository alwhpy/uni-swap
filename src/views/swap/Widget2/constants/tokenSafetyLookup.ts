...
class TokenSafetyLookupTable {
  ...
  update(lists: ListsState) {
    ...
    lists.byUrl[UNI_EXTENDED_LIST]?.current?.tokens.forEach((token: { address: string }) => {
    ...
    lists.byUrl[UNI_LIST]?.current?.tokens.forEach((token: { address: string }) => {
      this.dict[token.address.toLowerCase()] = TOKEN_LIST_TYPES.UNI_DEFAULT
    })

    // TODO: Figure out if this list is still relevant
    brokenTokenList.tokens.forEach((token: { address: string }) => {
...
const table = new TokenSafetyLookupTable()
