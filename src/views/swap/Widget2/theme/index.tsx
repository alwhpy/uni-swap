...
import { getAccent2, getNeutralContrast } from './utils'

import { darkTheme, ThemeColors } from './colors'
// import { useIsDarkMode } from './ThemeToggle'
...
export function getTheme(darkMode: boolean, overriddenColors?: Partial<ThemeColors>) {
  const [colors] = darkMode ? [darkTheme] : [darkTheme]
...
export function ThemeProvider({ children, ...overriddenColors }: PropsWithChildren<Partial<ThemeColors>>) {
  ...
  const themeObject = useMemo(() => getTheme(true, overriddenColors), [false, JSON.stringify(overriddenColors)])
